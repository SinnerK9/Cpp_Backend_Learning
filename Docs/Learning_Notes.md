...
## implementation：可变参数认识
日志的输出可能发生在每一个阶段：在WebServer，Timer等等模块中都可能出现需要输出日志的情况，这给输出日志这种操作带来了复杂性：对于不同的模块，它们需要的输出参数往往是不一样的，输出字符串中可能包含int，size_t等多种类型，多变的参数类型使得简单的模板编程难以适应需求，我们用以解决这个问题的方法是可变参数。
可变参数的方法在标准库<cstdarg>中。对于一个可变参数函数，它首先**至少有一个固定参数**，后续不确定的可变参数由...表示，**放在参数列表最后**，且**最多一个**。
**va_list是用来存放可变参数遍历状态的工具**。通过**va_start**进行初始化：传入的参数包括最后一个固定参数last_arg和一个va_list指针（设名为ap），由于函数调用时，参数从右到左入栈，而栈从高地址向低地址增长，最后一个固定参数和后续的所有可变参数的位置是连续的，这使得通过最后一个固定参数定位可变参数成为可能。
va_arg(ap,type)用于以type类型读取ap指向的参数，并且自动推进ap。
va_list指针在使用完毕后必须进行收尾，方法是va_end(ap)。

## 2026.5.2-5.3 模块化Logger类：日志系统
